...
struct Coord {
    ...
    bool const operator<(const Coord &o) const { return x < o.x || (x == o.x && y < o.y); }
};
typedef Coord coord;
struct Line {
    coord start;
...
class Vent {
    ...
    public:
        Vent() {};
        Vent(line p) { start = p.start; end = p.end; };
        Vent(coord s, coord e) { start = s; end = e; };
...
int main() {
    vector<Vent> myVents = readVents("../2021/5/input.txt");
